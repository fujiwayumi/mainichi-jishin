...
YOUTUBE_VIDEOS = {
    "bousai": [
        {"id": "3p7xwJq429w", "title": "津波防災啓発ビデオ「津波からにげる」（内閣府防災）"},
        {"id": "G7MVRUGkAyg", "title": "津波防災啓発動画「津波からにげる」（内閣府防災）"},
        {"id": "AeQEVKV2btA", "title": "内閣府防災 防災啓発動画"},
        {"id": "2tYpUXiw4-0", "title": "富士山の大規模噴火と降灰の影響（内閣府防災）"},
    ],
    "kyoukun": [
        {"id": "kCaQ9e6Rd8w", "title": "東日本大震災 発生時から翌朝まで NHK報道まとめ"},
        {"id": "crvTR1NOycA", "title": "首都直下地震への警鐘 能登半島地震で見えた地下のリスク（NHKスペシャル）"},
    ],
    "kazan": [
        {"id": "2tYpUXiw4-0", "title": "富士山の大規模噴火と降灰の影響（内閣府防災）"},
    ],
    "kagaku": [
        {"id": "qgjZHORXf50", "title": "南海トラフ地震対策編（内閣府防災）"},
        {"id": "YK7aGBsEKWU", "title": "南海トラフ地震どうなる？どうする？完全版（内閣府防災）"},
        {"id": "UR-CUwU_oXg", "title": "南海トラフ地震 時間差で起こりうる地震への備え（内閣府防災）"},
        {"id": "AjlbQiUkDU4", "title": "南海トラフ巨大地震（内閣府防災）"},
        {"id": "crvTR1NOycA", "title": "MEGAQUAKE 巨大地震 軟弱地盤 新たな脅威（NHKスペシャル）"},
    ],
    "occult": [
        {"id": "crvTR1NOycA", "title": "首都直下地震への警鐘（NHKスペシャル）"},
    ],
    "kaigai": [
        {"id": "kCaQ9e6Rd8w", "title": "東日本大震災 発生時から翌朝まで NHK報道まとめ"},
        {"id": "3p7xwJq429w", "title": "津波防災啓発ビデオ「津波からにげる」（内閣府防災）"},
...
# ===================================================
THEME_COLORS = {
    "bousai":  {"bg": "#1565C0", "accent": "#BBDEFB", "icon": "🛡️"},
    "kyoukun": {"bg": "#4E342E", "accent": "#FFCCBC", "icon": "📖"},
    "kazan":   {"bg": "#BF360C", "accent": "#FFCCBC", "icon": "🌋"},
    "kagaku":  {"bg": "#1B5E20", "accent": "#C8E6C9", "icon": "🔬"},
    "occult":  {"bg": "#4A148C", "accent": "#E1BEE7", "icon": "🌀"},
    "kaigai":  {"bg": "#006064", "accent": "#B2EBF2", "icon": "🌏"},
}
...
def generate_content_eyecatch(theme_id: str, title: str) -> str:
    ...
    c      = THEME_COLORS.get(theme_id, {"bg": "#37474F", "accent": "#CFD8DC", "icon": "📰"})
    bg     = c["bg"]
    accent = c["accent"]
    icon   = c["icon"]
    ...
    svg = f'''<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="auto" viewBox="0 0 {W} {H}" style="display:block;max-width:100%;">
  <defs>
  ...
  <circle cx="950" cy="150" r="280" fill="white" fill-opacity="0.04"/>
  <circle cx="950" cy="150" r="180" fill="white" fill-opacity="0.04"/>
  ...
  <!-- アイコン（大） -->
  <text x="80" y="320" font-family="sans-serif" font-size="160">{icon}</text>

  <!-- タイトル -->
  <text x="360" y="260" font-family="sans-serif" font-size="52" font-weight="bold"
        fill="white">{_esc(t1)}</text>'''

    if t2:
        svg += f'''
  <text x="360" y="330" font-family="sans-serif" font-size="52" font-weight="bold"
        fill="white">{_esc(t2)}</text>'''
    ...
    svg += f'''
  <!-- 日付 -->
  <text x="360" y="420" font-family="sans-serif" font-size="36"
        fill="{accent}" opacity="0.9">{date_str}</text>
